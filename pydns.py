...
import dns
from part_1 import question_to_bytes
from part_2 import DNSQuestion, DNSRecord, DNSPacket
...
def build_query(domain_name, id, record_type):
    name = dns.encode_dns_name(domain_name)
    header = dns.DNSHeader(id=id, num_question=1, flags=0)
    question = dns.DNSQuestion(name=name, type_=record_type, class_=CLASS_IN)
    return dns.header_to_bytes(header) + dns.question_to_bytes(question)
...
def parse_dns_packet(data):
    ...
    print(len(questions), len(answers), len(authorities), len(additionals))
...
def send_query(ip_address, domain_name, record_type):
    ...
    query = dns.build_query(domain_name, id, record_type)
