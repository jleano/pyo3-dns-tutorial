...
impl Buffer {
    ...
    fn slice(&mut self, length: usize) -> Bytes {
        // TODO convert length to <T>
    ...
    fn read_u16(&mut self) -> Result<u16, std::io::Error> {
        let out = self
            .buffer
            .slice(self.curindex..self.curindex + 2)
            .as_ref()
            .read_u16::<BigEndian>();
        self.curindex += 2;
        out
    ...
    fn read_u32(&mut self) -> Result<u32, std::io::Error> {
        let out = self
            .buffer
            .slice(self.curindex..self.curindex + 4)
            .as_ref()
            .read_u32::<BigEndian>();
        self.curindex += 4;
        out
...
fn parse_header(_py: Python, input: &[u8]) -> DNSHeader {
    let mut buffer = Buffer {
        buffer: Bytes::from(input.to_owned()),
        curindex: 0,
    };
...
fn _decode_name_simple(buffer: &mut Buffer) -> Vec<u8> {
    ...
    loop {
        let length = match buffer.get() {
            ...
            None => {
                println!("Error parsing simple name.");
                break;
            }
        };
        if length == 0 {
